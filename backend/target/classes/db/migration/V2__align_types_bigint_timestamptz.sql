-- Align schema types with JPA entities
-- Convert SERIAL/INT PKs & FKs to BIGINT, and timestamps to TIMESTAMPTZ
...
-- 3) Foreign keys to BIGINT
ALTER TABLE IF EXISTS productos ALTER COLUMN id_categoria TYPE BIGINT;
ALTER TABLE IF EXISTS productos ALTER COLUMN id_proveedor TYPE BIGINT;
ALTER TABLE IF EXISTS ventas ALTER COLUMN id_cliente TYPE BIGINT;
ALTER TABLE IF EXISTS detalle_ventas ALTER COLUMN id_venta TYPE BIGINT;
ALTER TABLE IF EXISTS detalle_ventas ALTER COLUMN id_producto TYPE BIGINT;
ALTER TABLE IF EXISTS inventario_movimientos ALTER COLUMN id_producto TYPE BIGINT;

-- 4) Timestamps to TIMESTAMPTZ (assume UTC)
